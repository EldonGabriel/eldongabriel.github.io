...
This report documents the implementation and analysis of system resource monitoring on an Ubuntu Server environment. The objective was to monitor CPU, memory, and swap usage using Linux command-line tools. The project focused on using native utilities to collect real-time performance information and validate system resource availability.

Several monitoring tools were tested using different output and filtering options to improve troubleshooting visibility and establish a baseline for normal system performance. The result was a validated monitoring workflow that supports system administration, troubleshooting, capacity planning, and security monitoring.
...
* **CPU and Process Monitoring**
    ...
    * Used `top -u` to filter processes by specific users to isolate impact.
...
The monitoring configuration was validated by checking command accuracy and confirming successful output generation.
...
* Using `free -h -t` combines formatting and calculation, making it a "stress test" for the utility's basic functionality.
* The `vmstat -w` command prevents column misalignment on high-resolution displays.
...
* Use consistent measurement units during reporting and analysis to avoid calculation errors in capacity planning.
* Establish baseline system performance values for accurate deviation detection.
* Ensure that swap space is active and factored into the total available memory before launching high-load applications.
* Regularly use validation commands such as `free -h -t` to prevent human error during manual calculations.
